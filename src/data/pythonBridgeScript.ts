 ...
 * Users can run this on their local system (Windows / Mac / Linux).
 * It listens on localhost:5000 and executes real OS actions (opening apps, shutdown, memory, etc.)
 */
...
class JarvisBridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/command":
            ...
            print(f"[BRIDGE] Received command from web UI: '{cmd}'")
...
def start_server():
    ...
    try:
        httpd = ServerClass(server_address, JarvisBridgeHandler)
    except Exception as e:
